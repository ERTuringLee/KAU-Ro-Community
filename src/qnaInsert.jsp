...
<%
	...
	Statement st = null;
	ResultSet rs =null;
	String savePath=application.getRealPath("/LEE-LIM/qna");
	...
	try{
		...
		st.executeUpdate("insert into qna (title, name,content, date,hits, file,password) values('"+title+"','"+writer+"','"+content+"',"+date+",0,'qna/"+newFile.getName()+"', '"+password+"')");
	...
	st.close();
	conn.close();
	response.sendRedirect("qna.jsp?id="+insertid);
%>