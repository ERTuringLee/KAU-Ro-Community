...
<%
	...
	Statement st = null;
	ResultSet rs =null;
	String savePath=application.getRealPath("/LEE-LIM/homestay");
	...
	try{
		...
		st.executeUpdate("insert into homestay (title, content,name, date,hits, file,password) values('"+title+"','"+content+"','"+writer+"',"+date+",0, 'homestay/"+newFile.getName()+"', '"+password+"')");
	...
	st.close();
	conn.close();
	response.sendRedirect("homestay.jsp?id="+insertid);
%>