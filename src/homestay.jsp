				...
				<div align="center">
					<%
						...
						try{
							...
							if(writer==null){
								if(idNum==null){
									rs = st.executeQuery("select * from homestay order by id DESC");
									...
									while(rs.next()){
										...
										out.println("</tr>");
										out.println("<tr>");
										out.println("<td>"+hid+"</th>");
										out.println("<td><a href='homestay.jsp?id="+hid+"'>"+title+"</td>");
									...
									out.println("<br>");
								%>
							<input style='float:right;margin-right:10px;' type='button' value='글쓰기' onclick='location.href="homestay.jsp?writer=yes"'>
								<%}else{
									rs = st.executeQuery("select * from homestay where id="+Integer.parseInt(idNum));
									while(rs.next()){
									 	hid =  rs.getInt("id");
							      		title = rs.getString("title");
							      		name = rs.getString("name");
							      	...
							      	st.executeUpdate("update homestay set hits='"+hits+"' where id="+Integer.parseInt(idNum));
							     %>
							     <div align="left">
							     	&nbsp;&nbsp;<input type="button" value="목록보기" onclick="location.href='homestay.jsp'">&nbsp;&nbsp;
							     	<!-- <input type="button" value="수정" id="homestay-modify" onclick="location.href='homestayModify.jsp?id='+<%=Integer.parseInt(idNum)%>">&nbsp;&nbsp; -->
							     	<input type="button" value="삭제" id="homestay-delete">
							     </div>
							     <br/>
							     <script>
							     	$(document).ready(function (){
							     		$('#homestay-delete').click(function () {
							     			<%
							     				rs = st.executeQuery("select * from homestay where id="+Integer.parseInt(idNum));
							     				while(rs.next()){
							     			...
							     			if(receivePassword=="<%=password%>"){
							     				alert("삭제가 완료되었습니다.")
							     				location.href ="homestayDelete.jsp?id="+"<%=Integer.parseInt(idNum)%>";
							     			}else{
							     				alert("비밀번호가 틀렸습니다.")
							     			}
							     		})
							     		$('#homestay-modify').click(function () {
							     			location.href ="homestayModify.jsp?id="+"<%=Integer.parseInt(idNum)%>";
							     		})
							     ...
							     rs = st.executeQuery("select * from homestay order by id DESC");
									...
									while(rs.next()){
										...
										out.println("<tr>");
										out.println("<td>"+hid+"</th>");
										out.println("<td><a href='homestay.jsp?id="+id+"'>"+title+"</td>");
										out.println("<td>"+name+"</td>");
										if((date%100)<10){
											String day="";
											day += "0"+date%100;
							...
							%>
								<form action="homestayInsert.jsp" method="post"  id='homestayform' enctype="multipart/form-data" >
							<%
								rs = st.executeQuery("select * from homestay");
								while(rs.next()){
									hid=  rs.getInt("id");
								}
								hid+=1;
