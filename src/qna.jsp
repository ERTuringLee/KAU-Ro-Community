			...
			<div id="left-section">
				...
				<%
						...
						try{
							...
							if(writer==null){
								if(idNum==null){
									rs = st.executeQuery("select * from qna order by id DESC");
									...
									while(rs.next()){
										...
										out.println("</tr>");
										out.println("<tr>");
										out.println("<td>"+qid+"</th>");
										out.println("<td><a href='qna.jsp?id="+qid+"'>"+title+"</td>");
									...
									out.println("<br>");
								%>
							<input style='float:right;margin-right:10px;' type='button' value='글쓰기' onclick='location.href="qna.jsp?writer=yes"'>
								<%}else{
									rs = st.executeQuery("select * from qna where id="+Integer.parseInt(idNum));
									while(rs.next()){
									 	qid =  rs.getInt("id");
							      		title = rs.getString("title");
							      		name = rs.getString("name");
							      	...
							      	st.executeUpdate("update qna set hits='"+hits+"' where id="+Integer.parseInt(idNum));
							     %>
							     <div align="left">
							     	&nbsp;&nbsp;<input type="button" value="목록보기" onclick="location.href='qna.jsp'">&nbsp;&nbsp;
							     	<!-- <input type="button" value="수정" id="qna-modify">&nbsp;&nbsp; -->
							     	<input type="button" value="삭제" id="qna-delete">
							     </div>
							     <br/>
							     <script>
							     	$(document).ready(function (){
							     		$('#qna-delete').click(function () {
							     			...
							     			if(receivePassword=="<%=password%>"){
							     				alert("삭제가 완료되었습니다.")
							     				location.href ="qnaDelete.jsp?id="+"<%=Integer.parseInt(idNum)%>";
							     			}else{
							     				alert("비밀번호가 틀렸습니다.")
							     			}
							     		})
							     		$('#qna-modify').click(function () {
							     			location.href ="qnaModify.jsp?id="+"<%=Integer.parseInt(idNum)%>";
							     		})
							     ...
							     rs = st.executeQuery("select * from qna order by id DESC");
									...
									while(rs.next()){
										...
										out.println("<tr>");
										out.println("<td>"+qid+"</th>");
										out.println("<td><a href='qna.jsp?id="+id+"'>"+title+"</td>");
										out.println("<td>"+name+"</td>");
										if((date%100)<10){
											String day="";
											day += "0"+date%100;
							...
							%>
								<form action="qnaInsert.jsp" method="post"  id='qnaform' enctype="multipart/form-data" >
							<%
								...
								out.println("<table border='1' style='border-collapse:collapse;border-left:1px solid #FFFFFF;border-right:1px solid #FFFFFF; width:100%;' rules='rows' >");
								out.println("<tr>");
								out.println("<th style='width:10%;'><input type='text' hidden name='id' value='"+qid+"'>제목</th>");
