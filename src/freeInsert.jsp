...
<%
	...
	try{
		...
		st.executeUpdate("insert into board (title, content,date,hits,recommand, nonRecommand, writer, file,password) values('"+title+"','"+content+"',"+date+",0,0,0,'"+writer+"', 'board/"+newFile.getName()+"', '"+password+"')");
	...
	st.close();
	conn.close();
	response.sendRedirect("free.jsp?id="+insertid);
%>