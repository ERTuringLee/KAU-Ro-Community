...
<%
	...
	try{
		...
		st.executeUpdate("insert into image (title, content,writer, date,hits, file,password, recommand, nonRecommand) values('"+title+"','"+content+"','"+writer+"',"+date+",0, 'image/"+newFile.getName()+"', '"+password+"', 0, 0)");
	...
	st.close();
	conn.close();
	response.sendRedirect("image.jsp?id="+insertid);
%>